...
## 19. Security-review tooling mistaken for authority (Strix)

Asset: the authority boundary and third-party targets. Attacker or control failure: an agent (or an operator reading marketing text) treats a security-review preflight as permission to scan. Path: the worker constructs a Strix review proposal and concludes that a successful preflight authorizes execution against a target. Impact: unauthorized scanning of a system the operator does not own — a legal and operational consequence, not just a technical one. Mitigation: the preflight is proposal-side by construction and always returns `execution_authorized: false`; caller ownership, authorization and checkout fields are untrusted claims; the exact effect proposal must additionally pass an independent AssuranceStore approval and an effect-gate ALLOW, and results are interpreted strictly with coverage-bound completeness (`src/security/strix-review.mjs`; tests: `tests/strix-review.test.mjs`, `tests/effect-gate.test.mjs`). The upstream is a pinned contract only — `usestrix/strix@2cc816781438f2993bcbb5c8cf3f693c25380142`, license `Apache-2.0`, no vendored code and no installed executable. Limitation: OPEN — exact written target authorization is a human artifact this repository cannot verify; it can only refuse to proceed without one. This release ships **no real Strix execution**: every live-scan claim is `NOT_EXECUTED`.
